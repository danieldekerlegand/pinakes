...
category id (plus the loaded :class:`~culturescrape.explorer.data.Corpus`) to
that operator playbook: the copy-pasteable ``culturescrape run`` /
...
from culturescrape.explorer.data import Corpus
from culturescrape.orchestrate.schedule import REFRESH_LOG_NAME

#: Export subdirectories under a job output root, mirroring the names
#: :mod:`culturescrape.orchestrate.corpus` writes them under (kept local so the
...
def _commands(category_id: str, job_spec: str, output_root: str) -> tuple[Command, ...]:
    ...
    return (
        command(
            ...
            f"culturescrape run {job_spec}",
        ),
        command(
            "Scheduled refresh",
            f"refresh categories not rebuilt in the last {DEFAULT_REFRESH_WINDOW}",
            f"culturescrape run --since {DEFAULT_REFRESH_WINDOW} {job_spec}",
        ),
        command(
            "Package",
            "package the built corpus into a publishable archive",
            f"culturescrape package {output_root}",
        ),
    )
...
def _find_manifest(job_root: Path) -> str | None:
    ...
    ``culturescrape package`` writes ``<name>-manifest.json`` beside its archive;
