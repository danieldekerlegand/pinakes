...
Stitching several dump domains with the LinguaScrape convergence export (US-004)
...
* a deity typed ``Concept`` by the myth-religion blueprint vs ``Deity`` by
  LinguaScrape (``cs:concept:Q146007`` vs ``cs:deity:Q146007``);
* a script typed ``Language`` by the language blueprint's writing-systems category
  vs ``WritingSystem`` by LinguaScrape (``cs:language:Q145625`` vs
  ``cs:writing-system:Q145625``);
* a geographic **place hub** the linker mints for a QID that LinguaScrape already
...
:func:`~culturescrape.schema.merge.merge_rows` machinery (which unions their label
...
LOGGER = logging.getLogger("culturescrape.ontology.reconcile_qid")
...
def reconcile_shared_qids(
    ...
    *,
    ...
    :func:`~culturescrape.schema.merge.merge_rows` (they share the QID, so they
...
def _redirect_edges(edges: Sequence[Edge], nodes: Sequence[Node]) -> list[Edge]:
    ...
    through :func:`~culturescrape.schema.merge.merged_csid_remap`; an edge whose
