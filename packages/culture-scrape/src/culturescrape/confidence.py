...
This mirrors the repo-root ``shared/confidence-rubric.json`` that the LinguaScrape
TypeScript side reads; the values are kept in lockstep by ``tests/test_confidence.py``
(which reads that JSON when the monorepo checkout is present). culture-scrape is a
