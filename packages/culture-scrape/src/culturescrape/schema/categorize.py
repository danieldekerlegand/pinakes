...
* a :data:`MEMBER_OF_CATEGORY` edge from every node to the **category node** for
  the :class:`~culturescrape.acquire.categories.CategorySpec` it was scraped
  under, and
...
deterministically (:func:`~culturescrape.schema.ids.mint_csid`), so a re-run
...
SYNTHETIC_SOURCE = "culturescrape"
