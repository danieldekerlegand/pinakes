...
Every node and edge row is classified by [`orchestrate/tiers.py`](../src/culturescrape/orchestrate/tiers.py)
...
culturescrape run jobs/merged-tiered.yml
```
...
Each tier is graded against its own [`GateThresholds`](../src/culturescrape/orchestrate/qa.py).
The defaults ([`tiers.DEFAULT_TIER_GATES`](../src/culturescrape/orchestrate/tiers.py))
...
`fail_on_violation` (the default for `culturescrape run`).
