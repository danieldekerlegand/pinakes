...
LinguaScrape corpus, what it cost, and what it surfaced.
...
| | |
|---|---|
| Corpus | `export/culturescrape` (gitignored), from `docs/culturescrape-export-manifest.json`: **6,667 nodes / 5,713 edges**, 17 node types, 7 edge types |
| Datalog export | `culturescrape to-datalog export/culturescrape --engine both --rules` → 35,191 projected facts; `graph.pl` (2.9 MB) + `graph.dl` (8.4 KB) + `*.facts` |
...
(`npx tsx scripts/export-for-culturescrape.ts` from the repo root), then
`culturescrape to-datalog ... --engine both --rules --out <dir>` and load/run as
below.
