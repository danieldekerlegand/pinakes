...
`culturescrape serve` launches a small, **read-only** web app over a corpus you
...
```sh
culturescrape serve tests/fixtures/explorer-corpus
...
copy-pasteable commands to rebuild (`culturescrape run jobs/…`), scheduled-refresh
(`culturescrape run --since 7d …`), and package it, the locations of its Neo4j and
