...
This page is the reference for the normalization layer (`src/culturescrape/schema/`):
...
|---|---|---|---|
| `csid:ID` | id | identity | culture-scrape global id; primary key, unique across all nodes |
...
Traceback (most recent call last):
    ...
culturescrape.schema.headers.SchemaError: property 'confidence' must be float, got int
...
```python
>>> from culturescrape.schema import NodeSchema, render_node_header
...
```python
>>> from culturescrape.schema import encode_value, decode_value
...
```python
>>> from culturescrape.schema import encode_values, decode_values
...
```python
>>> from culturescrape.schema import mint_csid
...
>>> from culturescrape.schema import map_records, merge_rows
