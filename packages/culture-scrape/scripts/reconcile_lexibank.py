...
The corpus build must have run first::
    ...
    uv run culturescrape run jobs/lexibank.yml     # acquire → normalize → link
...
``culturescrape.schema.lexibank_reconcile`` (type-checked, unit-tested); this script
...
from culturescrape.schema.lexibank_reconcile import (
    COGNATE_EDGE_FILE,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.nodes.is_file():
        print(
            ...
            "run `uv run culturescrape run jobs/lexibank.yml` first.",
            file=sys.stderr,
        )
        return 1
