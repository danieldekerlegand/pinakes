...
A fake :class:`~culturescrape.acquire.http.Transport` stands in for the network
...
from culturescrape.acquire import HttpClient, HttpResponse
...
def test_user_agent_header_is_sent(tmp_path: Path) -> None:
    ...
    sleeps: list[float] = []
    client = _client(
        tmp_path, transport, sleeps, user_agent="culture-scrape/test (contact)"
    )

    client.get("https://example.org/q")

    _, _, _, headers = transport.calls[0]
    assert headers["User-Agent"] == "culture-scrape/test (contact)"
