"""LinguaScrape cross-domain correlation rules over the merged fixture (US-005).

The bundled example dataset (``datalog/examples/dataset``) now carries
LinguaScrape-origin facts (``source: linguascrape``): a haplogroup, a Proto-Celtic
...
* the LinguaScrape facts *project* into the graph with their provenance intact;
...
# --- projection carries the LinguaScrape provenance -------------------------


def test_dataset_projects_linguascrape_origin_facts() -> None:
    facts = _dataset_facts()
    linguascrape = [f for f in facts if f.source == "linguascrape"]
    assert linguascrape, "the merged fixture projected no LinguaScrape-origin facts"
    # The correlation base predicates all carry the LinguaScrape provenance.
    for predicate in ("originates_from", "spoken_in", "descends_from"):
        assert any(f.predicate == predicate for f in linguascrape), predicate
...
def test_ancestor_closure_runs_over_linguascrape_descent() -> None:
    facts = _dataset_facts()
...
def test_correlation_smoke_skip_is_logged_when_swipl_absent(
    ...
    with caplog.at_level(logging.INFO):
        logging.getLogger(__name__).info(
            "skipping LinguaScrape correlation smoke test: swipl not found"
        )
