"""Tests for the offline reconciliation of LinguaScrape-origin rows.

Unlike :mod:`test_schema_reconcile` (which drives the Wikidata *network*
reconciler), these exercise the offline cascade that settles a LinguaScrape
...
def test_language_code_tier_matches_across_spellings() -> None:
    ...
    report = reconcile_linguascrape([incoming], [_existing()])
...
def test_name_region_tier_matches_without_a_code() -> None:
    ...
    report = reconcile_linguascrape([incoming], [existing])
...
def test_two_existing_nodes_sharing_a_code_are_ambiguous() -> None:
    ...
    report = reconcile_linguascrape([_incoming()], [rival_a, rival_b])
...
def test_fuzzy_ambiguity_lists_all_rival_candidates() -> None:
    ...
    report = reconcile_linguascrape([incoming], [rival_a, rival_b])
...
def test_fuzzy_threshold_is_configurable() -> None:
    ...
    loose = reconcile_linguascrape([incoming], [existing], fuzzy_threshold=0.5)
    strict = reconcile_linguascrape([incoming], [existing], fuzzy_threshold=0.99)
...
def test_matched_merge_preserves_both_provenance_and_identity() -> None:
    ...
    incoming = _incoming(source="linguascrape", source_url="")
    report = reconcile_linguascrape([incoming], [existing])
    ...
    assert merged["source"] == "wikidata;linguascrape"
    assert merged["source_url"] == "https://wd/Q1"
    # LinguaScrape's round-trip id is retained
    assert merged[LINGUASCRAPE_ID_KEY] == "pie"
...
def test_decision_is_recorded_in_overflow_for_matched_and_new() -> None:
    ...
    report = reconcile_linguascrape([matched_in, new_in], [_existing()])
