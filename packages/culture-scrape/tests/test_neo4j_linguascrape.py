"""Smoke test: the merged corpus loads LinguaScrape-origin rows under the same
...
LinguaScrape-origin (``source='linguascrape'``) rows in the **same** per-label
...
Because native and LinguaScrape rows share a file per label/type, "loaded under
...
# --------------------------------------------------------------------------- #
# A merged corpus: native + LinguaScrape rows share each label/type file.
# --------------------------------------------------------------------------- #
...
_LANGUAGES: tuple[dict[str, str | list[str]], ...] = (
    ...
    {
        # LinguaScrape-origin language, same label as the native row.
        ":LABEL": ["Language"],
...
_CULTURES: tuple[dict[str, str | list[str]], ...] = (
    ...
    {
        # LinguaScrape-origin cultures, same label as the native row.
        ":LABEL": ["ArchaeologicalCulture"],
...
_EDGES: tuple[dict[str, str], ...] = (
    {
        # Native edge — and a cross-source one: it points at a LinguaScrape node.
    ...
    {
        # LinguaScrape-origin edge, same :TYPE as the native one.
...
def _write_merged_corpus(root: Path) -> None:
    """Write the merged corpus with native + LinguaScrape rows sharing files."""
...
def test_merged_corpus_loads_linguascrape_under_shared_labels(tmp_path: Path) -> None:
    """Native + LinguaScrape nodes appear under the same labels + Entity anchor."""
    ...
    assert ls_culture["source"] == "linguascrape"
...
def test_merged_corpus_loads_shared_edge_types(tmp_path: Path) -> None:
    ...
    assert {"wikidata", "linguascrape"} <= sources

    # The cross-source edge (native start -> LinguaScrape end) is present.
    cross = (
...
def test_merged_corpus_has_globally_unique_csids(tmp_path: Path) -> None:
    """No csid collides between native and LinguaScrape rows in the merge."""
    corpus = tmp_path / "corpus"
    _write_merged_corpus(corpus)
