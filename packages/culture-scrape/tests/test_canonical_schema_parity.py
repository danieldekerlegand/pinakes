...
data contract and says not to fork it, but :mod:`culturescrape.schema.headers`
...
:mod:`culturescrape.translation`) renders the *contract's* header, so a
...
from culturescrape.schema.mapper import OVERFLOW_KEY, node_schema

#: The monorepo's shared contract, absent in a standalone culture-scrape checkout.
CANONICAL_SCHEMA_JSON = (
    Path(__file__).resolve().parents[3] / "shared" / "canonical-schema.json"
)
...
# --- culture-scrape's extensions stay outside the canonical tuple ------------
...
def test_acquisition_extensions_hang_off_the_canonical_prefix() -> None:
    ...
    unmapped-field overflow) are culture-scrape's, not the contract's. Keeping
...
def _engine_tsv_headers() -> tuple[str, str]:
    ...
    from culturescrape.translation import graph_json, to_tsv
