/**
 * Corpus quality / coverage / trust-tier scoring over `data/source/lexicons`.
 *
 * Lived at `server/services/data-quality-scorer.ts` until the cutover
 * (tasks/chief/80-cutover.json US-2) deleted the Express backend. It is pure
 * TSV analysis — no Express, no HTTP — and its remaining consumers are the two
 * report generators beside it (`scripts/coverage-report.ts`,
 * `scripts/corpus-tier-report.ts`), so it moved here rather than dying with the
 * server.
 *
 * The serving side is now Python: `services/api/src/pinakes/analytics/quality.py`
 * answers `/api/data-quality`. The two implementations are held together by the
 * committed artifacts they both reproduce — `docs/coverage-report.json` and
 * `docs/corpus-tier-report.json`, asserted here by `data-quality-scorer.test.ts`
 * and there by `services/api/tests/test_data_quality.py`.
 */
