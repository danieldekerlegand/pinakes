 ...
 *
 * Lived at `server/services/canonical-edges.ts` until the cutover
 * (tasks/chief/80-cutover.json US-2) deleted the Express backend. It is pure
 * TSV analysis, and `scripts/export-for-engine.ts` — which writes
 * `build/corpus/` — is its remaining consumer, so it moved here rather than
 * dying with the server. The serving side is now Python
 * (`services/api/src/pinakes/lexicons/canonical_edges.py`, behind
 * `/api/relationships/*`), graded by `services/api/tests/test_canonical_edges.py`
 * against the same corpus this module's own test reads.
 */
