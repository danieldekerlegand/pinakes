...
function rows(): GraphEntityRow[] {
  return sharedGraphAdapter.unwrap(SNAPSHOT);
}

describe("sharedGraphAdapter.unwrap", () => {
  ...
  it("tolerates a malformed / empty response", () => {
    expect(sharedGraphAdapter.unwrap(null)).toEqual([]);
    expect(sharedGraphAdapter.unwrap({})).toEqual([]);
    expect(sharedGraphAdapter.unwrap({ nodes: [], edges: [] })).toEqual([]);
  });
});

describe("sharedGraphAdapter.project — each declared dimension", () => {
  const proj = () => sharedGraphAdapter.project(rows(), {});

  it("declares all five dimensions", () => {
    expect(sharedGraphAdapter.dimensions).toEqual([
      "relational",
      "temporal",
      "spatial",
      "hierarchical",
...
describe("sharedGraphAdapter.project — filtering", () => {
  it("filters by the entityType facet", () => {
    const { categorical } = sharedGraphAdapter.project(rows(), {
  ...
  it("drops links whose endpoints are filtered out", () => {
    const { relational } = sharedGraphAdapter.project(rows(), {
      facetFilters: { entityType: "Region" },
    });
...
describe("registry integration", () => {
  it("is registered so it appears in the dataset picker", () => {
    const registered = ADAPTERS.find((a) => a.id === "pinakes-graph");
