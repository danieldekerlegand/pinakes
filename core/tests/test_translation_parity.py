...
``to_csv`` is the one conversion with no pre-migration counterpart --- culture-scrape
...
def test_neo4j_import_csv_matches_the_canonical_tsv_cells() -> None:
    ...
    (``--delimiter='\\t'``) and culture-scrape never rendered a CSV document. So the
...
def test_round_trip_skips_are_gated_only_on_external_binaries() -> None:
    ...
    (:data:`culturescrape.translation._MISSING`), so "the lib is missing" must never
