...
Every `culturescrape …` command below is exercised offline by the smoke test in
`tests/test_quickstart.py`, so the steps cannot drift from the tool.

## 1. Install

culture-scrape is a Python package (≥ 3.11). Clone it, make a virtualenv, and
install it editable with the dev and Neo4j extras:

```sh
git clone https://github.com/your-org/culture-scrape.git
cd culture-scrape
...
```sh
culturescrape generate blueprints/example.yml --out categories --job jobs/example.yml
```
...
`culturescrape run` drives a whole job — acquire → normalize → link → validate →
QA → export — and stitches every category into one connected corpus. The shipped
seed job spans five domains:

```sh
culturescrape run jobs/seed-corpus.yml
```
...
```sh
culturescrape fetch categories/peruvian-dishes.yml --out out/raw
```
...
```sh
culturescrape normalize out/raw/peruvian-dishes.jsonl \
...
```sh
culturescrape validate out/normalize
```
...
```sh
culturescrape link datalog/examples/dataset --out out/linked
```
...
```sh
culturescrape to-neo4j datalog/examples/dataset --out out/neo4j
```
...
```sh
culturescrape to-datalog datalog/examples/dataset --engine both --rules --out out/datalog
```
