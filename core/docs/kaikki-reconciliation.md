...
`kaikki` JSONL adapter (`src/culturescrape/acquire/kaikki.py`) reads such an extract and
...
- **Category / job:** `categories/kaikki.yml`, `jobs/kaikki.yml`
  (`uv run culturescrape run jobs/kaikki.yml`).
...
Reconciling the committed fixture (`uv run culturescrape run jobs/kaikki.yml` then
