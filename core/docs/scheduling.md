...
build it. `culturescrape run --since <duration>` is the scheduled-refresh mode —
...
15 3 * * *  cd /srv/culture-scrape && /srv/culture-scrape/.venv/bin/culturescrape run --since 24h jobs/seed-corpus.yml >> /var/log/culturescrape/refresh.log 2>&1
```

## Example: weekly refresh with a systemd timer

`/etc/systemd/system/culturescrape-refresh.service`:

```ini
[Unit]
Description=Refresh the culture-scrape seed corpus
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
WorkingDirectory=/srv/culture-scrape
ExecStart=/srv/culture-scrape/.venv/bin/culturescrape run --since 7d jobs/seed-corpus.yml
```

`/etc/systemd/system/culturescrape-refresh.timer`:

```ini
[Unit]
Description=Weekly culture-scrape corpus refresh
...
```sh
systemctl enable --now culturescrape-refresh.timer
systemctl list-timers culturescrape-refresh.timer   # confirm the next run
```
