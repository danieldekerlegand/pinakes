...
:func:`~pinakes_engine.datalog.export.export_dataset`, whose fact clauses the
embedded agora translation engine renders (pinakes:50 US-2). The second pair is
what ``pinakes_engine to-datalog --rules`` actually ships.
"""
...
from pinakes_engine.datalog.equivalence import Tuple
...
def test_the_lib_emitted_programs_carry_the_engines_clauses_and_a_live_closure(
    ...
    2. **Non-vacuity** — every relation the cross-engine check compares has a
       ...
       :func:`~pinakes_engine.datalog.materialize.materialize`. Without it a
