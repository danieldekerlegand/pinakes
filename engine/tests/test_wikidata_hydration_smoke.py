...
:data:`~pinakes_engine.acquire.wikidata_hydration.LANGUAGE_PROFILE` upgrade holds
...
present (built with ``pinakes_engine build-slice``; see
...
from pinakes_engine.acquire.wikidata_dump_index import default_index_path, load_index
from pinakes_engine.acquire.wikidata_hydration import LANGUAGE_PROFILE, hydrate_entity
...
def _resolve_slice() -> Path | None:
    ...
    honours ``PINAKES_ENGINE_WIKIDATA_SLICE`` (an explicit path), else the newest
