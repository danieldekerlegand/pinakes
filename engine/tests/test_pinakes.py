...
lifted out of the row (``source`` stamped ``pinakes``, a blank
...
_NODE_HEADER = (
    "csid:ID\t:LABEL\tname\tpinakes_id\tlanguage_code\t"
    "source\tsource_url\tsource_query\tretrieved_at\tconfidence:float"
)
_EDGE_HEADER = (
    ":START_ID\t:END_ID\t:TYPE\tweight:float\tpinakes_id\t"
...
def test_reads_the_committed_fixture_export() -> None:
    ...
    assert {r.provenance.source for r in records} == {PINAKES_SOURCE}


def test_node_row_maps_columns_and_keeps_the_pinakes_id_alias() -> None:
    ...
    assert aap.fields["pinakes_id"] == "aap"  # round-trip alias retained
...
def test_provenance_is_lifted_out_of_the_fields() -> None:
    ...
    assert aap.provenance.source == PINAKES_SOURCE
...
def test_source_name_is_overridable() -> None:
    records = _fetch(_spec(str(FIXTURE_EXPORT), {"source": "pinakes-2024"}))
    assert {r.provenance.source for r in records} == {"pinakes-2024"}
...
def test_per_row_license_column_is_lifted_and_overrides_the_param(
    ...
    root = _write_export(
        ...
        nodes={
            "language": f"{header}\n"
            "cs:language:xx\tLanguage\tX\txx\tpinakes\t\t\t\t0.5\tCC0-1.0"
        },
    )
...
def test_confidence_defaults_when_blank(tmp_path: Path) -> None:
    root = _write_export(
        ...
        nodes={
            ...
            "cs:language:xx\tLanguage\tX\txx\txx\tpinakes\t\t\t\t"
        },
    )
...
def test_empty_cells_are_dropped_from_fields(tmp_path: Path) -> None:
    root = _write_export(
        ...
        nodes={
            "language": f"{_NODE_HEADER}\n"
            "cs:language:xx\tLanguage\tX\txx\t\tpinakes\t\t\t\t0.5"
        },
    )
    (record,) = _fetch(_spec(str(root)))
...
def test_only_one_of_nodes_or_edges_need_be_present(tmp_path: Path) -> None:
    root = _write_export(
        ...
        edges={
            ...
            "cs:x\tcs:y\tDESCENDS_FROM\t\t\tpinakes\t\t\t0.9"
        },
    )
    (record,) = _fetch(_spec(str(root)))
...
def test_missing_path_is_rejected() -> None:
    with pytest.raises(PinakesExportError, match="no export path"):
        _fetch(_spec(None))
...
def test_non_directory_root_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(PinakesExportError, match="is not a directory"):
...
def test_a_non_canonical_header_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(PinakesExportError, match="invalid canonical header"):
...
def test_a_row_with_the_wrong_column_count_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(PinakesExportError, match="columns, header has"):
        _fetch(_spec(str(root)))
...
def test_factory_selects_the_adapter_via_params() -> None:
    spec = _spec(str(FIXTURE_EXPORT), {"adapter": "pinakes-export"})
    ...
    assert isinstance(adapter, PinakesExportAdapter)
