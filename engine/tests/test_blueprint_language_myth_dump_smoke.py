...
* :func:`~pinakes_engine.orchestrate.merge.write_merged_job` assembles a job over
  a language + myth subset (dump mode) plus the committed pinakes fixture
  export;
* :func:`~pinakes_engine.orchestrate.corpus.build_corpus` acquires + stitches it
...
* the merged corpus is schema-valid, carries both native dump nodes and
  ...
  (:func:`~pinakes_engine.neo4j.merge_load.verify_idempotent_load`), and reconciles
...
from pinakes_engine.orchestrate.generate import DumpSource
from pinakes_engine.orchestrate.jobs import load_job
from pinakes_engine.orchestrate.manifest import build_manifest
from pinakes_engine.orchestrate.merge import write_merged_job
from pinakes_engine.schema.lexicon_reconcile import reconcile_corpus_against_lexicon
...
def _resolve_slice() -> Path | None:
    ...
    ``PINAKES_ENGINE_WIKIDATA_SLICE``, else the newest slice in the conventional dir.
    """
...
def test_merged_corpus_reconciles_against_curated_lexicon(
    ...
    from pinakes_engine.schema.lexicon_reconcile import read_corpus_nodes
