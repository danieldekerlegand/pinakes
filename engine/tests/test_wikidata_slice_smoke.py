...
(``pinakes_engine build-slice ...``; see ``docs/wikidata-dump-runbook.md``). CI and
...
from pinakes_engine.acquire.wikidata_slice import MANIFEST_FORMAT
...
def _resolve_slice() -> Path | None:
    ...
    Honours ``PINAKES_ENGINE_WIKIDATA_SLICE`` (an explicit path); otherwise looks
