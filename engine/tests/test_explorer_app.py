...
from pinakes_engine import cli, translation  # noqa: E402
from pinakes_engine.explorer import create_app  # noqa: E402
from pinakes_engine.explorer.datalog import Datalog  # noqa: E402
...
def test_overview_lists_the_corpus_and_views() -> None:
    ...
    body = response.text
    assert "pinakes-engine explorer" in body
...
def test_category_actions_surface_commands_referencing_the_category() -> None:
    ...
    assert "pinakes_engine run jobs/" in body
    assert "pinakes_engine run --since 7d jobs/" in body
    assert "pinakes_engine package" in body
...
def test_datalog_console_projects_its_facts_through_the_agora_engine(
    ...
    assert len(rendered) == 1, (
        ...
        "translation engine (pinakes_engine.translation.dataset_datalog was not "
    ...
    # ...and the structure around them is still pinakes-engine's.
...
def test_datalog_deep_link_prefills_a_goal_for_the_csid() -> None:
    ...
    client = TestClient(create_app(FIXTURE_ROOT))
    from pinakes_engine.explorer.links import datalog_goal
