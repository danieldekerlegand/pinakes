...
graph by running a fixed sequence of ``pinakes_engine`` commands. This test runs
...
def test_cli_is_installed() -> None:
    """§1 — ``pinakes_engine --help`` exits cleanly and lists the run command."""
...
def test_run_the_pipeline_one_command(tmp_path: Path, offline: None) -> None:
    """§3 — ``pinakes_engine run jobs/seed-corpus.yml`` builds the full corpus.
...
def test_normalize_and_validate_a_category(tmp_path: Path) -> None:
    ...
    # `pinakes_engine validate <dir>` accepts the freshly normalized dataset.
    assert cli.main(["validate", str(out)]) == 0


def test_link_the_sample_corpus(tmp_path: Path) -> None:
    """§3 — ``pinakes_engine link`` infers cross-dimensional edges and metrics."""
    out = tmp_path / "linked"
...
def test_import_to_neo4j(tmp_path: Path) -> None:
    """§4 — ``pinakes_engine to-neo4j`` writes an admin-import script offline."""
    out = tmp_path / "neo4j"
...
def test_export_to_datalog(tmp_path: Path) -> None:
    """§5 — ``pinakes_engine to-datalog`` projects the corpus to both engines."""
