...
(`pinakes_engine.acquire.build_adapter`) maps that type to the concrete adapter
...
[`acquire/wikidata_hydration.py`](../src/pinakes_engine/acquire/wikidata_hydration.py).
...
```sh
pinakes_engine index-wikidata data/wikidata/latest-all.json.gz
...
`pinakes_engine.acquire.wikidata_dump.iter_entities`.
...
from pinakes_engine.acquire import DumpReadStats, iter_entities
...
`pinakes_engine enrich` command gives that corpus the dump's depth **without
...
```sh
pinakes_engine enrich out/seed-corpus data/wikidata/latest-all.json.gz \
...
[`NamedInLinker`](../src/pinakes_engine/ontology/named_in.py). Only registered edge
...
[`acquire/wikidata_enrich.py`](../src/pinakes_engine/acquire/wikidata_enrich.py).
