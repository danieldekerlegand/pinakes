...
It documents *how to drive* the converter (`src/pinakes_engine/neo4j/`); for *what*
...
The converter is the only part of pinakes-engine that talks to a graph database,
so its driver is kept out of the core install behind an extra:

```sh
pip install 'pinakes_engine[neo4j]'
```
...
```sh
pinakes_engine to-neo4j ./dataset --out ./dataset
...
>>> from pinakes_engine.neo4j.admin_import import build_command
>>> cmd = build_command(
...
```sh
pinakes_engine to-neo4j ./dataset --mode loadcsv
...
```pycon
>>> from pinakes_engine.neo4j.load_csv import generate_load_script  # doctest: +SKIP
...
```pycon
>>> from pinakes_engine.neo4j.constraints import constraint_statements
...
```pycon
>>> from pinakes_engine.neo4j.constraints import generate_constraints_script
...
```pycon
>>> from pinakes_engine.neo4j.constraints import label_constraint_statements
...
```sh
pinakes_engine from-neo4j --out ./roundtrip
...
```pycon
>>> from pinakes_engine.neo4j.counts import NODE_COUNT_QUERY
...
```pycon
>>> from pinakes_engine.neo4j.queries import iter_queries, lint_query
