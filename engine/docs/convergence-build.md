...
This is **the** operational runbook for the pinakes ↔ pinakes-engine convergence:
...
Python engine under `core/` (paths below are relative to that package
root unless noted). The design-level round trip is
[`engine-integration.md` §8](../../docs/engine-integration.md).

| # | Phase | Command | Detail |
|---|---|---|---|
| 1 | **Export** the live lexicons → canonical TSV | `npx tsx scripts/export-for-engine.ts` *(repo root)* | [full corpus (US-001)](#the-full-publishable-corpus-us-001) |
| — | **Reconcile** dry-run (optional preview) | `npx tsx scripts/reconciliation-report.ts` *(repo root)* | [§8](../../docs/engine-integration.md) |
| 2 | **Build** the corpus (acquire → normalize → reconcile/link → export) | `uv run pinakes_engine run jobs/pinakes-full.yml --force` *(--force after a re-export; stage fingerprints skip a stale corpus otherwise)* | [US-001](#the-full-publishable-corpus-us-001) |
| — | **Validate + QA** the output | `uv run pinakes_engine validate out/pinakes-full/corpus` | [US-001](#the-full-publishable-corpus-us-001) |
| 3 | **Publish** the versioned artifact (`.tar.gz` + SHA-256 manifest) | `uv run pinakes_engine package out/pinakes-full/corpus --out dist --name pinakes-full-corpus` | [US-001](#the-full-publishable-corpus-us-001) |
| 4 | **Load** into Neo4j (constraints/indexes + idempotent MERGE) | `uv run pinakes_engine to-neo4j out/pinakes-full/corpus --mode loadcsv` | [US-002](#load-the-corpus-into-neo4j-us-002) |
| — | **Smoke** the load (counts by type) | `uv run pinakes_engine neo4j-counts` | [US-002](#load-the-corpus-into-neo4j-us-002) |
| 5 | **Materialize** Datalog inference | `uv run pinakes_engine datalog-materialize out/pinakes-full/corpus` | [US-004](#materialize-datalog-inference-at-scale-us-004) |
...
- **CI runs the fixture path only** (offline, deterministic): `pinakes_engine run
...
```bash
cd core
pinakes_engine run jobs/pinakes.yml
```
...
`pinakes-export` adapter. `pinakes_engine run` sees a full-pipeline job (all of
...
```bash
pinakes_engine validate out/pinakes/corpus
pinakes_engine qa out/pinakes/corpus --min-provenance-completeness 0 --fail-on-violation
```
...
#    repo-root build/corpus/{nodes,edges}). Run from the repo root:
...
cd core
uv run pinakes_engine run jobs/pinakes-full.yml --force

# 3. Validate + QA the output (the build already runs both; re-check independently):
uv run pinakes_engine validate out/pinakes-full/corpus
uv run pinakes_engine qa out/pinakes-full/corpus \
...
uv run pinakes_engine package out/pinakes-full/corpus \
...
release = {
    "job": "pinakes-full",
    "built_from": "build/corpus (scripts/export-for-engine.ts) -> jobs/pinakes-full.yml",
...
uv run pinakes_engine to-neo4j out/pinakes-full/corpus --mode admin \
...
uv run pinakes_engine to-neo4j out/pinakes-full/corpus --mode loadcsv
...
```bash
uv run pinakes_engine neo4j-counts
...
```bash
cd core
uv run pinakes_engine to-datalog out/pinakes-full/corpus --engine both --rules \
...
```bash
uv run pinakes_engine datalog-materialize out/pinakes-full/corpus \
...
**Validation.** `pinakes_engine validate out/pinakes-full/corpus` (the schema/QA gate from
...
```bash
npm run dev:full        # app + pinakes-engine sidecar + Neo4j (needs Docker)
# in another shell:
...
8. **Python-side handling (only if needed):** bespoke reconcile/ontology logic for the new type
   lives under `core/` — see integration §10 and
...
- [`docs/engine-integration.md`](../../docs/engine-integration.md) — the
