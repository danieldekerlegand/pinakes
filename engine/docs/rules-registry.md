...
```
pinakes_engine rules-registry --regenerate
```
...
```
pinakes_engine rules-registry            # validate the committed registry + summary
pinakes_engine rules-registry --json out.json
```
...
1. `pinakes_engine rules-registry --regenerate` (rewrites `datalog/rules_registry.tsv`).
