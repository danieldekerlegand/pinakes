...
`core/.gitignore`); commit the manifest/measurements, never the
slice itself.
...
```bash
cd core
uv run pinakes_engine build-slice \
...
uv run pinakes_engine index-wikidata out/wikidata/wikidata-20260712-blueprint-slice.json.gz
```
...
`out/wikidata/` (or the path in `PINAKES_ENGINE_WIKIDATA_SLICE`) and asserts the
...
PINAKES_ENGINE_WIKIDATA_SLICE=out/wikidata/wikidata-20260712-blueprint-slice.json.gz \
...
uv run pinakes_engine generate blueprints/food-drink.yml \
  --dump  "$SLICE" \
...
uv run pinakes_engine run jobs/food-drink-dump.yml --workers 4
```
...
```bash
uv run pinakes_engine datalog-materialize out/food-drink/corpus \
...
lever is `pinakes_engine merge`, which expands N blueprints in dump mode and
appends a `pinakes-export` category, writing the single job whose categories
`pinakes_engine run` then stitches (`orchestrate/merge.py`).
...
uv run pinakes_engine merge blueprints/language.yml blueprints/myth-religion.yml \
  --dump  "$SLICE" \
  --index "$SLICE.index.sqlite3" \
  --hydrate default \
  --pinakes "$(cd ../.. && pwd)/build/corpus" \
...
uv run pinakes_engine run jobs/merged-dump.yml --workers 4

# 3. Prove the Neo4j load is idempotent (offline: MERGE double-load, no server).
uv run pinakes_engine neo4j-counts --dataset out/merged-dump/corpus
```
...
with the live pinakes export (`build/corpus`):
...
changed; the incremental path (`pinakes_engine sync-wikidata`) answers that by diffing
...
pinakes_engine sync-wikidata jobs/food-drink-dump.yml \
