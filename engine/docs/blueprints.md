...
**stub** per category. `pinakes_engine generate` expands every stub into a full,
...
`pinakes_engine.orchestrate.generate`; the category schema each stub expands to is
...
5. **Generate the categories and job:**
   ...
   ```sh
   pinakes_engine generate blueprints/<domain>.yml \
   ...
   spurious diffs). Run the domain with `pinakes_engine run jobs/<domain>.yml`.
