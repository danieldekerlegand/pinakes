...
fix a single engine-neutral fact shape — `pinakes_engine.datalog.Fact` — that
...
```console
$ pinakes_engine to-datalog <dir> --engine swipl|souffle|problog|both --rules --out <dir>
```
...
in `pinakes_engine.datalog.export`.
...
installed in CI (the `pinakes-engine` job in `.github/workflows/convergence-qa.yml`)
...
```python
>>> from pinakes_engine.datalog import Fact, Dialect, render_atom
...
in `src/pinakes_engine/ontology/`.
...
`pinakes_engine.datalog.node_file_facts` reads a node TSV file (`nodes/<type>.tsv`)
...
`pinakes_engine.datalog.edge_file_facts` reads an edge TSV file
...
```python
>>> from pinakes_engine.datalog import edge_facts, predicate_for_type
...
`pinakes_engine.datalog.write_souffle_program` writes a directory rather than a
...
```python
>>> from pinakes_engine.datalog import Fact, souffle_relations
...
```python
>>> from pinakes_engine.datalog import render_souffle_program
...
(`pinakes_engine.schema.tsvio`), so a field containing a tab, newline, or
...
the same syntax. `pinakes_engine.datalog.write_problog_program` emits a single
...
```python
>>> from pinakes_engine.datalog import AnnotatedFact, Fact, render_annotated_fact
...
program worth more than the TSV it came from. `pinakes_engine.datalog.RULES` is a
...
TypeScript engine, per `docs/engine-integration.md`.
...
[`pinakes_engine.acquire.taxonomy`](../src/pinakes_engine/acquire/taxonomy.py). It
...
[`datalog/taxonomy/subclass_of.tsv`](../src/pinakes_engine/datalog/taxonomy/subclass_of.tsv)
(`source`/`source_url`/`retrieved_at`/`confidence` per row, network-free in CI).
[`pinakes_engine.datalog.taxonomy`](../src/pinakes_engine/datalog/taxonomy.py)
...
[`pinakes_engine.acquire.constraints`](../src/pinakes_engine/acquire/constraints.py) fetches
...
artifact
[`datalog/constraints/property_constraints.tsv`](../src/pinakes_engine/datalog/constraints/property_constraints.tsv)
(network-free in CI).
[`pinakes_engine.datalog.constraints`](../src/pinakes_engine/datalog/constraints.py)
...
[`datalog/constraints/rules_registry.tsv`](../src/pinakes_engine/datalog/constraints/rules_registry.tsv)
...
```python
>>> from pinakes_engine.datalog.constraints import constraint_file_rules
...
replay artifact
[`datalog/schema/edge_constraints.tsv`](../src/pinakes_engine/datalog/schema/edge_constraints.tsv),
...
materialiser cannot express negation); `pinakes_engine schema-constraints <dataset>
...
```python
>>> from pinakes_engine.datalog import RULES, render_program, render_souffle_program
...
`pinakes_engine.datalog.examples`, and `tests/test_datalog_examples.py` runs each
one on that dataset in SWI-Prolog (skipping when `swipl` is absent):

```python
>>> from pinakes_engine.datalog.examples import EXAMPLES
...
```console
$ pinakes_engine to-datalog datalog/examples/dataset --engine swipl --rules --out /tmp/eg
...
because it is fast and needs no engine at all. `pinakes_engine.datalog.materialize`
...
counted, and validated without an engine. `pinakes_engine datalog-materialize <dataset>` prints
the base-relation counts the rules read and the derived-relation counts, and
`--json <path>` writes them as a manifest:

```python
>>> from pinakes_engine.datalog.export import collect_facts
>>> from pinakes_engine.datalog.examples import dataset_dir
>>> from pinakes_engine.datalog.materialize import summarize
...
committed at `datalog/rules_registry.tsv` and regenerated with `pinakes_engine
...
`validate_registry` is the QA gate (run in CI and by `pinakes_engine rules-registry`):
...
```python
>>> from pinakes_engine.datalog.registry import build_registry, validate_registry
