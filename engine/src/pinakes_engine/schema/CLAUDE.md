...
**A column pinakes-engine needs but the contract does not declare is an extension and
...
contract's 28 node / 13 edge columns, pinakes-engine emitted 28 *different* node / 8
...
- **GOTCHA — `merge_rows` fuzzy-name dedup will collapse distinct attribute facts.** The
  ...
  facts of ONE language stay dissimilar. Verify empirically: `pinakes_engine run` then
