...
* **edge ``:TYPE`` is registered** — every edge's ``:TYPE`` is a relationship
  type the formal ontology (``pinakes_engine.ontology``) defines, so a linker
...
from pinakes_engine.schema.headers import Column, SchemaError
from pinakes_engine.schema.tsvio import Row, TsvError, column_key, read_rows
...
def _check_edge_types(
    ...
    The formal ontology (``pinakes_engine.ontology``) is the single source of
