...
#. **map + normalize** — :func:`~pinakes_engine.schema.mapper.map_records` mints
   each row's ``csid``, stamps its label, and cleans every value into its
   canonical column;
#. **anchor** (optional) — :func:`~pinakes_engine.schema.anchor.anchor_rows`
   attaches Getty ids where a local dump is supplied;
#. **reconcile** (optional) —
   :func:`~pinakes_engine.schema.reconcile.reconcile_rows` resolves names to
   Wikidata QIDs where a reconciler is supplied;
#. **dedup** — :func:`~pinakes_engine.schema.merge.merge_rows` collapses
   duplicate entities to one node each;
#. **edges** — :func:`~pinakes_engine.schema.categorize.categorize_rows` links
...
from pinakes_engine.schema.anchor import GettyIndex, anchor_rows
from pinakes_engine.schema.categorize import categorize_rows
from pinakes_engine.schema.headers import EdgeSchema
from pinakes_engine.schema.ids import IdError, normalize_type
from pinakes_engine.schema.mapper import (
    map_insimul_records,
    map_pinakes_records,
    map_records,
    node_schema,
...
from pinakes_engine.schema.reconcile import WikidataReconciler, reconcile_rows
...
def read_raw_records(path: str | Path) -> list[RawRecord]:
    ...
    (the shape :func:`pinakes_engine.acquire.writer.record_to_jsonl` emits).
...
def normalize_records(
    ...
    *reconciler* is given; otherwise the run is offline and deterministic. The
    ...
    mapped via :func:`~pinakes_engine.schema.mapper.map_pinakes_records`,
...
def _normalize_insimul(records: Iterable[RawRecord]) -> NormalizationResult:
    ...
    :func:`~pinakes_engine.schema.mapper.map_insimul_records` — which preserves
    ...
    :mod:`pinakes_engine.acquire.insimul` while reading the artifact, so no dedup /
...
def _normalize_pinakes(
    ...
    Records are mapped via
    :func:`~pinakes_engine.schema.mapper.map_pinakes_records` — which re-mints
...
def _redirect_edges(edges: Sequence[Row], nodes: Sequence[Row]) -> list[Row]:
    ...
    Dedup (:func:`~pinakes_engine.schema.merge.merge_rows`) collapses duplicate
    ...
    merge would dangle and fail :mod:`~pinakes_engine.schema.validate` (breaking
