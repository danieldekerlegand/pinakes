...
:func:`~pinakes_engine.orchestrate.runner.run_job` drives each category through the
...
whole system works end to end. Given a :class:`~pinakes_engine.orchestrate.jobs.Job`
it
...
#. **links** the combined graph across every dimension
   (:func:`~pinakes_engine.ontology.run.run_linkers`), minting the place / period /
   language hub nodes that wire the domains together;
#. writes the canonical corpus TSV and **validates** it;
#. measures **connectivity** (:mod:`pinakes_engine.ontology.metrics`) and grades
   the **quality gates** (:mod:`pinakes_engine.orchestrate.qa`), failing the build
   if the corpus fragments below :data:`DEFAULT_MIN_COMPONENT_FRACTION` or a gate
   trips; and
#. generates the **Neo4j** bulk-import script
   (:mod:`pinakes_engine.neo4j.admin_import`) and the **Datalog** ``.pl`` / ``.dl``
   exports (:mod:`pinakes_engine.datalog.export`) for the corpus.
...
from pinakes_engine.ontology.reconcile_qid import reconcile_shared_qids
...
from pinakes_engine.orchestrate.manifest import build_manifest
...
from pinakes_engine.orchestrate.runner import (
    DEFAULT_WORKERS,
    AdapterFactory,
    JobRun,
    run_job,
...
from pinakes_engine.schema.pipeline import NormalizationResult, write_result
from pinakes_engine.schema.validate import validate_directory

#: Logger corpus builds emit their progress on.
LOGGER = logging.getLogger("pinakes_engine.orchestrate.corpus")
