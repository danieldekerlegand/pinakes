...
from pinakes_engine.orchestrate.qa import GateThresholds
...
class Job:
    ...
    Attributes:
        ...
        tiered_trust: When true, the corpus build classifies every row into a
            ...
            ``inferred`` — see :mod:`pinakes_engine.orchestrate.tiers`), writes a
        ...
        tier_gates: Per-tier QA threshold overrides, keyed by tier name; each
            value is a partial :class:`~pinakes_engine.orchestrate.qa.GateThresholds`
            mapping. A tier absent here uses its
            :data:`~pinakes_engine.orchestrate.tiers.DEFAULT_TIER_GATES` entry.
...
def _parse_tier_gates(
    ...
    value a mapping of :class:`~pinakes_engine.orchestrate.qa.GateThresholds`
    ...
    from pinakes_engine.orchestrate.tiers import ALL_TIERS
