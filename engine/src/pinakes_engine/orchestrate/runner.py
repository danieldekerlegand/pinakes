"""Drive a :class:`~pinakes_engine.orchestrate.jobs.Job` end to end.
...
:class:`~pinakes_engine.acquire.http.HttpClient`, whose per-host rate limit is
enforced globally (see :mod:`pinakes_engine.acquire.http`).
...
:mod:`pinakes_engine.orchestrate.fingerprint`.
...
from pinakes_engine.datalog.export import engines_for_choice, export_dataset
...
from pinakes_engine.orchestrate.catalog import update_catalog
from pinakes_engine.orchestrate.fingerprint import (
    StageManifest,
    combine,
...
from pinakes_engine.orchestrate.qa import (
    QA_REPORT_MD_SUFFIX,
    QA_REPORT_SUFFIX,
    QaPolicy,
    evaluate_directory,
...
LOGGER = logging.getLogger("pinakes_engine.orchestrate.runner")
...
def run_job(
    ...
    *qa* policy's quality gates are evaluated over it and a report is written to
    ...
    :mod:`pinakes_engine.orchestrate.qa`). By default the report is informational;
    ...
    :mod:`pinakes_engine.orchestrate.catalog`), so the corpus stays discoverable.
...
def _default_adapter_factory(job: Job) -> AdapterFactory:
    ...
    :class:`~pinakes_engine.acquire.http.HttpClient`, so its per-host rate limit
