...
1. **Diff** the fresher slice against the one the corpus was built from
   (:mod:`pinakes_engine.acquire.wikidata_diff`) → the added / changed / removed
...
3. **Re-hydrate & re-export** only those entities: a small *delta dump* holding
   ...
   allowlist), and :func:`~pinakes_engine.orchestrate.corpus.build_corpus` rebuilds
...
4. **Prove idempotency** offline: the delta MERGE-loads over the base corpus to a
   fixed point (:func:`~pinakes_engine.neo4j.merge_load.verify_upsert_load`).
...
category-granularity ``--since`` refresh log (:mod:`pinakes_engine.orchestrate.
...
from pinakes_engine.acquire.wikidata_diff import DumpDiff, diff_dumps, write_delta_dump
...
from pinakes_engine.neo4j.merge_load import (
    IdempotencyReport,
    verify_idempotent_load,
    verify_upsert_load,
)
from pinakes_engine.ontology.linker import Node
...
from pinakes_engine.schema.ids import csid_qid
...
def dataset_qids(dataset_dir: str | Path) -> frozenset[str]:
    ...
    (:func:`~pinakes_engine.schema.ids.csid_qid`); alias-/name-anchored nodes (no
...
def write_sync_log(
    ...
    The entity-granularity twin of :func:`~pinakes_engine.orchestrate.schedule.
...
def last_sync_at(output_root: str | Path) -> datetime | None:
    ...
    ``--since`` (:func:`~pinakes_engine.orchestrate.schedule.parse_duration`) to skip
