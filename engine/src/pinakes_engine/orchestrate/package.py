...
model inherits)" statement per class — see :mod:`pinakes_engine.schema.license_class`.
...
from pinakes_engine.schema.license_class import (
    class_registry,
    partition_by_class,
    redistribution_for,
)
...
def _assert_no_personal_tier(source: Path, members: Sequence[str]) -> None:
    ...
    * **synthetic** (insimul-bridge US-003, the Insimul bridge spec §7 "License
    ...
    :data:`~pinakes_engine.orchestrate.tiers.PERSONAL_SOURCES` or
    :data:`~pinakes_engine.orchestrate.tiers.SYNTHETIC_SOURCES` aborts the package.
...
def _type_counts(
    ...
    The corpus manifest (:class:`~pinakes_engine.orchestrate.manifest.CorpusManifest`)
