"""``pinakes_engine`` command-line entrypoint.

Exposes the acquisition engine as a CLI. ``pinakes_engine fetch <category.yml>
...
from pinakes_engine.acquire.categories import CategorySpecError, load_category
...
from pinakes_engine.acquire.run import run_acquisition
from pinakes_engine.acquire.wikidata import WikidataSparqlError
from pinakes_engine.acquire.wikidata_dump import DumpReadStats, WikidataDumpError
from pinakes_engine.acquire.wikidata_dump_index import build_index, default_index_path
...
from pinakes_engine.acquire.wikidata_hydration import UnknownProfileError, get_profile
...
from pinakes_engine.acquire.writer import record_to_jsonl
...
from pinakes_engine.datalog.materialize import MaterializeError, summarize
...
from pinakes_engine.neo4j.counts import count_summary
from pinakes_engine.neo4j.export import Neo4jExportError, export_to_tsv
from pinakes_engine.neo4j.load_csv import load_corpus
from pinakes_engine.neo4j.merge_load import verify_idempotent_load
...
from pinakes_engine.ontology.run import run_linkers, select_linkers
from pinakes_engine.orchestrate.catalog import load_catalog, render_table
from pinakes_engine.orchestrate.corpus import (
    CorpusBuild,
    CorpusBuildError,
    build_corpus,
...
from pinakes_engine.orchestrate.generate import BlueprintError, DumpSource, generate
from pinakes_engine.orchestrate.incremental import (
    UpsertResult,
    last_sync_at,
    run_upsert,
    write_sync_log,
)
from pinakes_engine.orchestrate.jobs import (
    STAGE_ORDER,
    Job,
    JobConfigError,
    load_job,
...
from pinakes_engine.schema.anchor import GettyIndex
from pinakes_engine.schema.categorize import CategorizeError
from pinakes_engine.schema.mapper import MapperError
from pinakes_engine.schema.pipeline import (
    NormalizationResult,
    PipelineError,
    normalize_records,
    read_raw_records,
...
from pinakes_engine.schema.reconcile import WikidataReconciler
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    if args.verify:
        ...
        from pinakes_engine.acquire.wikidata import fetch_count
    ...
    if result.job is not None:
        print(f"  job: {result.job} (run: pinakes_engine run {result.job})")
    return 0
...
def _cmd_serve(args: argparse.Namespace) -> int:
    try:
        from pinakes_engine.explorer import create_app, run_server
        from pinakes_engine.explorer.data import CorpusError
    except ImportError:
        return _fail(
            "the web explorer needs the 'gui' extra: "
            "pip install 'pinakes_engine[gui]'"
        )
