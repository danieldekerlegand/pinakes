...
* a :class:`LinkerRegistry` grouping linkers by their
  :class:`~pinakes_engine.ontology.registry.Dimension`;
...
from pinakes_engine.ontology.registry import Dimension, validate_type
from pinakes_engine.schema.tsvio import Row

#: A node or edge row, in the same shape :mod:`pinakes_engine.schema.tsvio` reads
...
class Linker(ABC):
    ...
    def link_full(
        ...
        :func:`~pinakes_engine.ontology.run.run_linkers`.
        """
...
def tag_inferred_edge(edge: Edge, linker_name: str) -> Edge:
    ...
    :func:`~pinakes_engine.ontology.run.run_linkers` so both stamp identically.
    """
