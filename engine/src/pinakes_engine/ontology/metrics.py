...
``edges/<type>.tsv`` (the shape :func:`pinakes_engine.schema.pipeline.write_result`
emits) and reports:
...
* **edges by dimension and by ``:TYPE``** — which kinds of links carry the
  ...
  (:mod:`pinakes_engine.ontology.registry`).
...
from pinakes_engine.ontology.registry import get
from pinakes_engine.schema.headers import SchemaError
from pinakes_engine.schema.tsvio import Row, TsvError, read_rows
...
#: :data:`pinakes_engine.acquire.pinakes.PINAKES_SOURCE`). Native edges
...
def read_dataset(directory: str | Path) -> tuple[list[Node], list[Edge]]:
    ...
    Files are classified the way :mod:`pinakes_engine.schema.validate` classifies
...
def nodes_by_label(nodes: Sequence[Node]) -> dict[str, int]:
    ...
    key :func:`pinakes_engine.schema.pipeline.write_result` files a node under — so
