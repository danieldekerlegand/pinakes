...
Each category is normalized on its own (``pinakes_engine.schema.pipeline`` produces
one :class:`~pinakes_engine.schema.pipeline.NormalizationResult` per
...
(:func:`pinakes_engine.schema.merge.merge_rows`), which unions their aliases and
...
from pinakes_engine.schema.categorize import CATEGORY_LABEL, MEMBER_OF_CATEGORY
from pinakes_engine.schema.merge import DEFAULT_FUZZY_THRESHOLD, merge_rows
from pinakes_engine.schema.pipeline import NormalizationResult

LOGGER = logging.getLogger("pinakes_engine.ontology.stitch")
...
def stitch_categories(
    ...
    *,
    ...
    :func:`~pinakes_engine.schema.merge.merge_rows` (the merge runs per ``csid`` so
...
def _merge_by_csid(nodes: Iterable[Node], fuzzy_threshold: float) -> list[Node]:
    ...
    :func:`~pinakes_engine.schema.merge.merge_rows`, so duplicates of one entity
