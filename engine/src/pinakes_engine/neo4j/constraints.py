...
Every pinakes-engine node carries the shared :data:`ENTITY_LABEL` label in
...
optional driver (see :mod:`pinakes_engine.neo4j`). ``IF NOT EXISTS`` makes every
...
from pinakes_engine.neo4j.admin_import import discover_dataset
...
def render_script(statements: tuple[str, ...]) -> str:
    ...
    return (
        "// Generated by pinakes_engine: schema constraints and indexes for Neo4j.\n"
...
def apply_constraints(
    ...
    *config*/*env* via :func:`pinakes_engine.neo4j.connect` and closed before
    returning.
