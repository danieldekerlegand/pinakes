...
orchestration — that the ``pinakes_engine graphrag-index`` CLI subcommand runs.

Two deliberate isolation rules, mirroring the rest of :mod:`pinakes_engine.neo4j`:
...
* The optional ``neo4j`` driver is only touched through an already-connected
  ...
  :func:`pinakes_engine.neo4j.connect`); importing this module never needs it.
...
class EmbedderNotInstalled(ImportError):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            "not installed. Install it with: pip install 'pinakes_engine[graphrag]'"
        )
...
def run_index_build(
    ...
    *,
    ...
    and the usual :class:`pinakes_engine.neo4j.Neo4jConfigError` /
    :class:`~pinakes_engine.neo4j.Neo4jDriverNotInstalled` when the graph is not
