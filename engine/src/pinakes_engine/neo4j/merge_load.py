...
The live load leg (:mod:`pinakes_engine.neo4j.load_csv`) is deliberately
...
smoke queries a real load would answer (:mod:`pinakes_engine.neo4j.counts`): a
node is tallied under **every** label it carries (its type ``:LABEL`` plus the
shared :data:`~pinakes_engine.neo4j.constraints.ENTITY_LABEL` anchor), so the
...
from pinakes_engine.neo4j.counts import CountSummary
...
class _MergeGraph:
    ...
    :mod:`pinakes_engine.neo4j.load_csv` merges on — so loading the same dataset
