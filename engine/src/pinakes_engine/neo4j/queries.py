...
* node :data:`DEFINED_LABELS` — the base ``Entity`` label every node carries
  (:mod:`pinakes_engine.neo4j.constraints`), the synthesized ``Category`` /
  ``Type`` labels (:mod:`pinakes_engine.schema.categorize`), and the node kinds
...
from pinakes_engine.schema.categorize import CATEGORY_LABEL, TYPE_LABEL
