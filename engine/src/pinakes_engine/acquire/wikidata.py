...
result binding to a :class:`~pinakes_engine.acquire.records.RawRecord`. A category
like *"every Peruvian dish"* is therefore a **query**, not a scrape: the SPARQL
text is the declarative definition of the set.

The query is sent through the shared :class:`~pinakes_engine.acquire.http.HttpClient`
...
def fetch_count(
    ...
    *,
    ...
    Used by ``pinakes_engine generate --verify`` to record each class's live
