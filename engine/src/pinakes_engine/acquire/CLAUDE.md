...
`HttpClient` (cached/retried/User-Agent-identified). CLI: `pinakes_engine build-slice`.
...
built once by `pinakes_engine index-wikidata <dump>` / `build_index()`. This replaced the retired
...
class-membership closure (`src/pinakes_engine/datalog/taxonomy.py` reads the
...
- Imports `SubclassEdge` from `pinakes_engine.datalog.taxonomy` (acquire→datalog is
