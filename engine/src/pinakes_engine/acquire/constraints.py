...
(:mod:`pinakes_engine.datalog.constraints`), but the constraints themselves are not in
...
:class:`~pinakes_engine.datalog.constraints.PropertyConstraint`.
...
* a *type* constraint's class QID → the node ``:LABEL`` it maps to
  (:data:`~pinakes_engine.acquire.taxonomy.CORPUS_CLASS_QIDS`, reused — blank when the
  class is not a corpus backing class).

The resolved constraints are written to the committed replay artifact
(:data:`~pinakes_engine.datalog.constraints.PROPERTY_CONSTRAINTS_TSV`) that CI reads
back,
never touching the network — the same network-free discipline the taxonomy extractor
(:mod:`pinakes_engine.acquire.taxonomy`) uses. Respect Wikimedia politeness on the
SPARQL path — it goes through the shared
:class:`~pinakes_engine.acquire.http.HttpClient`.
"""
...
from pinakes_engine.datalog.constraints import (
...
#: :data:`~pinakes_engine.acquire.taxonomy.CORPUS_CLASS_QIDS`): only ``:TYPE``\\ s with
#: a single, reliable backing property appear — a constraint is only as sound as this
