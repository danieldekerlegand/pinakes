...
(:mod:`pinakes_engine.datalog.rules`), but the ``subclass_of`` half is not in the
...
* the **on-disk dump index** (``P279`` edges, walked with
  :meth:`~pinakes_engine.acquire.wikidata_dump_index.DumpIndex.class_closure`) —
...
small, provenanced :class:`~pinakes_engine.datalog.taxonomy.SubclassEdge` set that
...
:class:`~pinakes_engine.acquire.http.HttpClient` (cached / retried / identified),
same as :mod:`pinakes_engine.acquire.wikidata_slice`.
"""
...
from pinakes_engine.acquire.wikidata_dump_index import DumpIndex
from pinakes_engine.datalog.taxonomy import (
    SUBCLASS_OF_COLUMNS,
    SubclassEdge,
)
