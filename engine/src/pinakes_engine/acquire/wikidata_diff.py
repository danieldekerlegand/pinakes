...
(``docs/data-model.md``; :mod:`pinakes_engine.neo4j.load_csv`), so an incremental
...
incremental path re-hydrates and re-exports (:mod:`pinakes_engine.orchestrate.
incremental`).
...
def write_delta_dump(
    ...
    exact ``latest-all`` framing :func:`~pinakes_engine.acquire.wikidata_dump.
