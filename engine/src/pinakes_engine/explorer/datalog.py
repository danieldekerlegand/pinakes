...
Mirrors :mod:`pinakes_engine.explorer.live`: the explorer reads the symbolic layer
...
reusing :mod:`pinakes_engine.datalog.examples` for both running and linting.
...
class DatalogExample:
    ...
    *slug* is the query file's stem, *title* and *focus* its documented summary
    (from :data:`pinakes_engine.datalog.examples.EXAMPLES`), and *source* the full
...
class Datalog:
    ...
    *swipl* defaults to the ``swipl`` on PATH; pass an explicit path to pin it, or
    ...
    def run(self, source: str) -> DatalogOutcome:
        ...
        :func:`pinakes_engine.datalog.examples.run_example`, exactly as a shipped
