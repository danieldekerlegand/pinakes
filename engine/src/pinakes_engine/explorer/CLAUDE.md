...
TS client (`server/services/engine-client.ts`) from the **same URL**. The helper
...
`engine-client.ts`; a change to a JSON body must update that schema and its vitest.
...
- **`/api/retrieve?q=&k=&depth=`** (GraphRAG, Phase 5.1) — hybrid retrieval: embed the
  ...
  mid-retrieve → 503. The TS proxy relies on this: `engine-client.requestJson` maps
  any `>=500` (so the 503) to `PinakesEngineUnavailableError`, which `graph.ts handleError`
