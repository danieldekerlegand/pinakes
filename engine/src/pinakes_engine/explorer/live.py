...
the live store. This module wraps :mod:`pinakes_engine.neo4j` so the rest of the
...
database, mirroring :mod:`pinakes_engine.neo4j`. The driver is only touched when a
...
from pinakes_engine.neo4j import (
    Neo4jConfigError,
    load_config,
)
from pinakes_engine.neo4j import (
...
class Connect(Protocol):
    """The shape of :func:`pinakes_engine.neo4j.connect`, injectable for tests."""
...
class Neo4jLive:
    ...
    resolved by :func:`pinakes_engine.neo4j.load_config`) and a *connect* function
    ...
    def run(
        ...
        Raises :class:`pinakes_engine.neo4j.Neo4jConfigError` when no settings are
        resolvable, :class:`pinakes_engine.neo4j.Neo4jDriverNotInstalled` when the
