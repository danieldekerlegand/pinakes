...
* **Confidence becomes probability.** An edge's ``confidence`` (the strength the
  :class:`~pinakes_engine.datalog.Fact` companion ``rel_conf/4`` carries) is lifted
...
evaluate identically in ProbLog, so :func:`pinakes_engine.datalog.render_rule` in
the :data:`~pinakes_engine.datalog.Dialect.PROLOG` dialect is reused unchanged.
...
from pinakes_engine.datalog import DatalogError, Dialect, Fact
from pinakes_engine.datalog.edges import edge_facts
from pinakes_engine.datalog.nodes import node_file_facts
from pinakes_engine.datalog.rules import ARITY, Rule, render_rule
from pinakes_engine.schema.headers import EdgeSchema
...
#: NOTE — the banner still reads ``culture-scrape``: it is a byte-for-byte
#: contract with the vendored agora ``translation_py`` engine, whose Rust
#: emitters hard-code that string. It can only change when that wheel is
#: rebuilt; the parity tests fail the moment the two sides disagree.
_HEADER = """\
...
def render_annotated_fact(annotated: AnnotatedFact) -> str:
    ...
    The clause body is the ordinary :func:`~pinakes_engine.datalog.render_fact`
...
def annotate_edge_group(facts: Iterable[Fact]) -> list[AnnotatedFact]:
    ...
    *facts* is the group :func:`~pinakes_engine.datalog.edge_facts` projects from a
...
def _problog_edge_file_facts(
    ...
    Mirrors :func:`~pinakes_engine.datalog.edge_file_facts` but groups each row's
...
def collect_problog_facts(
    ...
    Reuses :func:`~pinakes_engine.datalog.export.collect_facts` for discovery and
    ...
    (:func:`~pinakes_engine.datalog.export.tier_row_filter`; ``None`` keeps all).
...
def write_problog_program(
    ...
    *rendered_facts* supplies the annotated clauses already rendered, in
    ...
    translation engine delegates through (:mod:`pinakes_engine.translation`), which
