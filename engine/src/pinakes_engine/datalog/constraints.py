...
class. The corpus's edge vocabulary (:mod:`pinakes_engine.ontology.registry`) is built
...
:mod:`pinakes_engine.acquire.constraints` from a WDQS ``P2302`` query) and
...
never imports :mod:`pinakes_engine.acquire` (no cycle) and translates from resolved
names alone.
...
from pinakes_engine.datalog.rules import RULES, Rule
...
class ConstraintRule:
    ...
    *prolog_clauses* / *souffle_clauses* are the Horn clauses for each engine —
    ...
    def prolog_rule(self) -> Rule | None:
        """The :class:`~pinakes_engine.datalog.rules.Rule` for SWI-Prolog, or ``None``.
    ...
    def souffle_rule(self) -> Rule | None:
        """The :class:`~pinakes_engine.datalog.rules.Rule` for Soufflé, or ``None``."""
...
def translate(
    ...
    *,
    ...
    (defaults to the shipped :data:`~pinakes_engine.datalog.rules.RULES`).
    """
    if curated is None:
