...
* one **``<predicate>.facts`` file** per relation — the rows themselves, in
  ...
  (:func:`pinakes_engine.schema.tsvio.encode_value`) so a field containing a tab,
...
from pinakes_engine.datalog import Atom, DatalogError, Dialect, Fact, render_predicate
from pinakes_engine.datalog.rules import Rule, render_rule
from pinakes_engine.schema.headers import DELIMITER
from pinakes_engine.schema.tsvio import encode_value
...
def _atom_type(value: Atom) -> str:
    ...
    :func:`pinakes_engine.datalog.render_atom`: it is an ``int`` subclass in
...
def write_souffle_facts(
    ...
    *,
    ...
    strict TSV writer (:func:`pinakes_engine.schema.tsvio.encode_value`), so the
    ...
    *rendered_shards* maps a predicate to a shard body already rendered elsewhere
    ...
    (:mod:`pinakes_engine.translation`). Those bodies are written verbatim and only
