...
work behind the ``pinakes_engine to-datalog`` command — the single step from
...
(:data:`~pinakes_engine.datalog.SOUFFLE_PROGRAM_NAME`) plus one ``.facts`` file
...
library (:data:`~pinakes_engine.datalog.RULES`) to whichever program(s) are
written.
"""
...
from pinakes_engine.datalog import Dialect, Fact, edge_file_facts, node_file_facts
...
#: :func:`pinakes_engine.orchestrate.tiers.assert_no_personal_records`).
...
#: instead; see :func:`pinakes_engine.acquire.insimul.world_rule_entries`).
...
def tier_row_filter(tier: str | None) -> Callable[[Row], bool]:
    ...
    * ``None`` — the **public** program: every row of a *contained* tier is
      ...
      (:data:`~pinakes_engine.orchestrate.tiers.PERSONAL_SOURCES`) or Insimul
      worlds (:data:`~pinakes_engine.orchestrate.tiers.SYNTHETIC_SOURCES`) still
...
def collect_facts(
    ...
    *,
    ...
    (:mod:`pinakes_engine.datalog.taxonomy`) is appended as ``subclass_of/2`` facts
...
def export_dataset(
    ...
    *,
    ...
    :data:`~pinakes_engine.datalog.SOUFFLE_PROGRAM_NAME` (plus its ``.facts``
    files). When *include_rules* is set, the shared inference library
    (:data:`~pinakes_engine.datalog.RULES`) is attached to every program.

    When *include_constraints* is set, the rules translated from Wikidata property
    constraints (:mod:`pinakes_engine.datalog.constraints`, rules-layer US-002) are
    ...
    *include_rules*. The constraint rules are **engine-specific**: SWI-Prolog receives
    ...
    (:mod:`pinakes_engine.datalog.schema_constraints`, rules-layer US-003) are
    attached —
    ...
    *tier* scopes which trust tier reaches the program (:func:`tier_row_filter`).
    ...
    # translation engine (agora:60, via ``pinakes_engine.translation``): row
    ...
    # rules, the rule sections are pinakes-engine's own inference layer, and the
...
def _export_rule_bearing(
    ...
    *,
    ...
    :func:`pinakes_engine.translation.dataset_datalog`; each emitter is handed them
