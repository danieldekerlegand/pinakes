...
queries + offline linter). `pinakes_engine to-datalog … --rules` attaches `RULES`.
...
`to_json()` is what `pinakes_engine datalog-materialize <dataset> --json` writes.
...
- **The full-corpus report is a committed release record** (`docs/schema-constraints-report.json`,
  regenerate with `pinakes_engine schema-constraints build/corpus --json ...`), NOT a
...
- **Generated + committed, pinned by a test.** `build_registry()` aggregates the three
  ...
  rule/provenance change: `pinakes_engine rules-registry --regenerate` (a `_cmd_*`
...
- **The QA gate is `validate_registry(entries) -> [RegistryProblem]`** (run in CI +
  `pinakes_engine rules-registry`): parseable clauses (balanced; head + `pred(args)`/
...
As of US-001 both engines ARE installed where the suite runs: the `pinakes-engine`
