...
fact count. Facts render via :func:`pinakes_engine.datalog.render_fact` in the
:data:`~pinakes_engine.datalog.Dialect.PROLOG` dialect — unless the caller passes
...
(:mod:`pinakes_engine.translation`). Either way this module owns only the
...
from pinakes_engine.datalog.rules import (
    Rule,
    render_rule,
    rule_signatures,
    tabled_signatures,
...
def write_program(
    ...
    *rendered_facts* supplies the fact clauses already rendered, in fact-stream
    ...
    delegates through (:mod:`pinakes_engine.translation`). The clauses are then the
