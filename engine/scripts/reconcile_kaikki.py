...
The corpus build must have run first::
    ...
    uv run pinakes_engine run jobs/kaikki.yml     # acquire → normalize → stitch → link
...
The matching + tallying logic lives in ``pinakes_engine.schema.kaikki_reconcile``
...
from pinakes_engine.schema.kaikki_reconcile import (
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.wordform_nodes.is_file():
        print(
            ...
            "run `uv run pinakes_engine run jobs/kaikki.yml` first.",
            file=sys.stderr,
        )
        return 1
