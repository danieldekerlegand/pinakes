...
Acquisition (network) and the corpus build must have run first::
    ...
    uv run pinakes_engine run jobs/civilizations.yml     # acquire → normalize → stitch
...
``pinakes_engine.schema.lexicon_reconcile`` (type-checked, unit-tested); this script
...
from pinakes_engine.schema.lexicon_reconcile import (
    reconcile_corpus_against_lexicon,
    render_markdown,
)

#: Repo root = three levels up from this file (core/scripts/).
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.corpus_nodes.is_file():
        print(
            ...
            "run `uv run pinakes_engine run jobs/civilizations.yml` first.",
            file=sys.stderr,
        )
        return 1
