...
The corpus build must have run first::
    ...
    uv run pinakes_engine run jobs/typology.yml     # acquire → normalize → stitch
...
``pinakes_engine.schema.typology_reconcile`` (type-checked, unit-tested); this script is
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not any(path.is_file() for path in node_files.values()):
        print(
            ...
            "run `uv run pinakes_engine run jobs/typology.yml` first.",
            file=sys.stderr,
        )
        return 1
