...
The corpus build must have run first::
    ...
    uv run pinakes_engine run jobs/glottolog.yml     # acquire → normalize → stitch
...
``pinakes_engine.schema.glottolog_reconcile`` (type-checked, unit-tested); this
...
from pinakes_engine.schema.glottolog_reconcile import (
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.corpus_nodes.is_file():
        print(
            ...
            "run `uv run pinakes_engine run jobs/glottolog.yml` first.",
            file=sys.stderr,
        )
        return 1
