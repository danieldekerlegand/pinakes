...
`lexicons/civilizations.tsv` via `scripts/import-from-culturescrape.ts --add-cultures`.
...
- **Command** — `npx tsx scripts/import-from-culturescrape.ts --add-cultures`
...
**Known gap (for the US-006 pilot report):** `scripts/export-for-culturescrape.ts`
...
npx tsx scripts/import-from-culturescrape.ts --add-cultures
...
`export/culturescrape/writeback/civilizations-additions-report.json`.
