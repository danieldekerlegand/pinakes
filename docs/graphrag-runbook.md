...
**LinguaScrape proxy (browser-facing):** `GET /api/graph/retrieve?q=&k=&depth=`
(`server/routes/graph.ts`). The browser only talks to the LinguaScrape origin; this
...
# for /api/graph/retrieve on the LinguaScrape origin):
