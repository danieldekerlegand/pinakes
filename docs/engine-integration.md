# pinakes-engine ↔ pinakes: Data-Layer Convergence Plan
...
Make pinakes's data and pinakes-engine's data **one correlatable body of knowledge** —
...
pinakes-engine's Python pipeline.
...
- The ~91k-LOC React/Vite frontend stays TypeScript no matter what, so the rewrite buys nothing
  for the UI.
- Data production, reconciliation, and correlation belong in a pipeline — and pinakes-engine
...
**Key insight:** `cultural-lineages.tsv` is already a hand-built edge table. pinakes-engine
...
```
 ┌─────────── pinakes-engine (Python) — canonical data + correlation engine ───────────┐
...
The Python data/correlation engine is **first-party pinakes code at `engine/`**. It began as a
vendored copy of the standalone pinakes-engine repo (its 105-commit upstream history was
...
[`docs/REMOVED_FEATURES.md`](./REMOVED_FEATURES.md). The `pinakes_engine` package namespace,
...
extension behind the `pinakes_engine.translation` adapter; canonical format *rendering* is its
...
| US-006 | Provenance on 100% of exported rows + per-type coverage metric | `scripts/export-for-engine.ts` (`provenance` block) | §4.3, §7 |
| US-007 | Bidirectional write-back (graph → lexicons), conflict-safe, ambiguous-id-safe | `scripts/import-from-engine.ts` | §9 |
...
**5,836 edges** across 8 edge types — see [`docs/engine-export-manifest.json`](./engine-export-manifest.json)
and [`docs/reconciliation-report.json`](./reconciliation-report.json). The `build/corpus/`
...
```
 data/source/lexicons/*.tsv  (source of truth, human-curated)
      ...
      ▼
 build/corpus/nodes/<node-type>.tsv + edges/<edge-type>.tsv + manifest.json
      ...
      │
      │ 3. INGEST + RECONCILE + LOAD  (Python — engine/)
...
- **Steps 1, 2, 5, GATE are TypeScript in this repo** (`scripts/`). Step 3 is Python under
  `engine/`; step 4 is the pinakes app + the `graph-app-integration` PRD.
- **Step 3 is itself a one-command, offline, reproducible recipe:** `pinakes_engine run
  ...
  (`engine/docs/convergence-manifest.json`) asserted against a fresh build in
  ...
  runbook [`engine/docs/convergence-build.md`](../engine/docs/convergence-build.md).
...
5. **Regenerate the export & snapshots:**
   ...
   (`docs/engine-export-manifest.json`, `docs/reconciliation-report.json`) — the
...
8. **Python side:** if the new node/edge type needs bespoke reconcile/ontology handling, cross-link
   the work under `engine/` (see §10) — the tabular adapter ingests the new
...
[`convergence-build.md` "Add a new domain to the live graph"](../engine/docs/convergence-build.md).
...
- **Ingesting the export:** [`docs/reconcile-pinakes.md`](../engine/docs/reconcile-pinakes.md)
  — how the export flows through reconcile, and which side owns each merge decision.
- **Reconciliation cascade:** `src/pinakes_engine/schema/reconcile.py` (QID lookup) +
  `merge.py` (clustering/merge); see [`docs/data-model.md`](../engine/docs/data-model.md).
- **Typed import headers:** `src/pinakes_engine/schema/headers.py` — the canonical column contract
  in §4 deliberately mirrors these so the export is `neo4j-admin import`-clean.
- **Neo4j / Datalog:** [`docs/neo4j.md`](../engine/docs/neo4j.md),
  [`docs/datalog.md`](../engine/docs/datalog.md),
  [`docs/ontology.md`](../engine/docs/ontology.md).
- **Python-side Ralph PRDs:** `engine/ralph/` (acquisition, schema/entity-resolution,
...
`engine-client.ts` models — `/search` → `{ query, results[] }` (`SearchHit` rows),
`/metrics` → `pinakes_engine.ontology.metrics.to_json` (a corpus with no readable metrics
...
`engine/src/pinakes_engine/explorer/app.py` (`_wants_json`).
...
- **App + sidecar + Neo4j** — `npm run dev:full` (`scripts/dev-full.sh`). Starts the
  `pinakes_engine` and `neo4j` docker-compose services detached, waits for the sidecar health
...
- **Python-side convergence — tasklist 16:**
  ...
  and the vendored engine under [`engine/`](../engine/) —
  ingestion ([`docs/reconcile-pinakes.md`](../engine/docs/reconcile-pinakes.md)),
  Neo4j load ([`docs/neo4j.md`](../engine/docs/neo4j.md)) and Datalog
  ([`docs/datalog.md`](../engine/docs/datalog.md)). Use its own toolchain
