...
- **Pipeline (pure core + lazy heavy imports):** `ml/src/linguascrape_ml/finetune.py`
- **CLI:** `uv run linguascrape-finetune` (`ml/src/linguascrape_ml/train_finetune.py`)
...
them lazily inside the training/generation functions, so `import linguascrape_ml`
...
`linguascrape_ml.finetune.require_finetune_deps()` raises a clear, actionable error
if they are missing.
...
uv run linguascrape-finetune              # uses ml/configs/finetune-smoke.json
```
...
cd ml
uv pip install trl peft accelerate bitsandbytes
uv run linguascrape-finetune --config configs/finetune-gpu.json
```
