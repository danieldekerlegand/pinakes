...
- **Generator (pure core):** `ml/src/linguascrape_ml/kgqa.py`
- **CLI:** `uv run linguascrape-export-kgqa` (`ml/src/linguascrape_ml/export_kgqa.py`)
...
- **OPTIONAL Gemini-polish variant:** `ml/src/linguascrape_ml/polish_kgqa.py`
...
`linguascrape-polish-kgqa` produces a **clearly-separated** variant dataset: it
...
uv run --project ml linguascrape-export-kgqa            # add --no-mlflow to skip logging
