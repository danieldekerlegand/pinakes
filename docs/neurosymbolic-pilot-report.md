...
- **Task:** rule-guided link prediction on `DESCENDS_FROM` — a neural edge scorer
  ...
  training queries.
- **Scallop side:** `ml/src/linguascrape_ml/scallop_train.py` (US-003), the
...
- **DeepProbLog side:** `ml/src/linguascrape_ml/deepproblog_pilot.py` +
  `linguascrape-deepproblog` (US-004). DeepProbLog's inference **backend is ProbLog**;
...
`cd ml && uv run linguascrape-deepproblog` (needs the DVC-tracked triples splits;
`uv run --project ml dvc pull` first) and `uv run linguascrape-train-scallop` (US-003).
...
<!-- The scale-probe table below is regenerated by `linguascrape-deepproblog`; the
prose above is authored and preserved across regenerations. -->

<!-- DEEPPROBLOG-PROBE:START (generated by linguascrape-deepproblog) -->

### Measured scale probe (regenerated by `linguascrape-deepproblog`)
