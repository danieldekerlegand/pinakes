...
- **`curated`** — human-vetted through the LinguaScrape lexicon gate (`source = linguascrape`).
...
Because auto-admission **never writes `lexicons/*.tsv`**, the whole app-facing corpus is the
