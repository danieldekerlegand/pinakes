...
| *Convergence:* Python ingest/graph | Ralph `pinakes-convergence-python` (8) | @10cb3f1 |
...
surfaced live at `/api/data-quality`) — **14/15 domains meet their target**; the one exception
(`language-range-polygons`) is honestly flagged (Wikidata carries no inline range polygons for our
corpus — see US-006).

| Domain | Before | Now | Target | Status |
|---|---:|---:|---|:---:|
| civilizations | 89 | 170 | 150+ | ✅ met |
| archaeological sites | 151 | 550 | 500+ | ✅ met |
| archaeological cultures | 27 | 277 | 200+ | ✅ met |
| migration routes | 62 | 104 | 100+ | ✅ met |
| trade routes | 25 | 39 | expanded (Silk/Spice/Incense/…) | ✅ met |
| cuisines | 21 | 101 | 80+ | ✅ met |
| ingredient origins | 41 | 111 | 100+ | ✅ met |
| cooking techniques | 42 | 92 | 80+ | ✅ met |
| writing systems | 60 | 115 | 100+ | ✅ met |
| deities | 76 | 206 | 200+ | ✅ met |
| architectural styles | 20 | 90 | 90+ | ✅ met |
| dance traditions | 32 | 92 | 90+ | ✅ met |
| literary traditions | 12 | 62 | foundational corpus (50+) | ✅ met |
| myth motifs | 32 | 61 | 60+ | ✅ met |
| language range polygons | 133 | 133 | 200+ | ⚠️ under (no Wikidata polygons) |

Approach (delivered): **piloted one domain end-to-end first** (`data-population-pilot` —
civilizations), proving the acquisition → reconcile → write-back → graph → UI pipeline lands real
data; then scaled the proven pipeline across domains (`data-population`) behind a curation +
attribution + QA gate (Guiding Principle #8: academic credibility — every row keeps its
source/license/provenance). Sources & per-domain counts: [`docs/ATTRIBUTION.md`](ATTRIBUTION.md);
schema + inventory: [`docs/DATA_SCHEMA.md`](DATA_SCHEMA.md).
...
*Last updated: July 8, 2026 — Phases 7–14 + convergence + `operationalize-graph` complete; **data population at scale complete** (§15 — 14/15 domains at target, full corpus rebuilt & loaded). Next priority: **production-verification pass** (see §16).*