/**
 * Shared culture-scrape graph as a UnifiedExplorer dataset (US-008).
 *
 * Unlike the local TSV datasets, this adapter fetches a `{ nodes, edges }`
 * snapshot from `/api/graph/overview` (server/routes/graph.ts → Neo4j) and
 * projects the generic graph payload into every explorer dimension:
 ...
 *
 * The node/edge shapes mirror `server/services/graph-store.ts` (re-used from the
...
export const culturescrapeAdapter: DatasetAdapter<GraphEntityRow> = {
  id: "culturescrape-graph",
