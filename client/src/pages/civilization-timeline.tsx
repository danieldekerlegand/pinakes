...
export default function CivilizationTimeline() {
  ...
  const colorFn = useCallback((item: TimelineItem) => {
    return getFamilyColor(item.groupName, 0.7);
  }, []);

  const buildTooltip = useCallback((item: TimelineItem): TooltipData => {
    const meta = item.metadata ?? {};
    return {
      id: item.id,
      name: item.name,
      nativeName: meta.nativeName as string | undefined,
      type: 'family',
      region: item.groupName,
      timeOrigin: formatYear(item.startYear),
      timeEnd: formatYear(item.endYear),
      politicalStructure: meta.politicalStructure as string | undefined,
      capital: meta.capital as string | undefined,
      totalSpeakers: meta.population as number | undefined,
      writingSystems: meta.writingSystems as string[] | undefined,
    };
  }, []);
