...
| Story | Deliverable |
|-------|-------------|
| US-001 | This workspace + MLflow infrastructure (DVC since removed — `docs/artifact-versioning.md`) |
...
```bash
cd ml
uv sync --extra dev          # create .venv + install torch/pykeen/problog/mlflow/…
...
## Dataset & artifact versioning — none (DVC removed)

> **DVC was removed** in the flatten's Phase 0. There is no `dvc pull`, no
> `dvc add`, no remote. Sections below that used to end in "then re-pin the data"
> now just end. Rationale + how to re-enable DVC (with a *cloud* remote), git-lfs,
> or release artifacts: [`docs/artifact-versioning.md`](../docs/artifact-versioning.md).

Large build outputs are plain **git-ignored directories, regenerated on demand**
from committed inputs. Nothing pins their content hashes:
...
The exporters are deterministic and byte-stable, so a rebuild from unchanged
inputs reproduces the same tree — that property is what the committed manifests
and snapshot tests ratchet on, and it is why losing the pins is survivable.
...
```bash
cd ml
# needs ml/data/triples/* — run pinakes-export-triples first (nothing to pull)
...
#   ml/data/embeddings/<model>/entity_embeddings.npy (+ metadata.json; git-ignored)
```
...
- **Reproducible by construction:** a pinned seed + the committed `entities.tsv`/
  ...
  corpus or hyperparameters change. The committed doc records the corpus md5 (a
  historical DVC hash — a provenance label, no longer resolvable) and the
  split-manifest sha256 the metrics were measured on.
...
└── data/                    # regenerable datasets/artifacts (git-ignored)
