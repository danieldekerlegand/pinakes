...
installing the (undeclared) training stack and pulling the DVC data::
    ...
    uv run linguascrape-finetune            # local smoke (tiny model, MPS/CPU)
    # or a specific config:
    uv run linguascrape-finetune --config configs/finetune-gpu.json

It loads a committed JSON config (:class:`~linguascrape_ml.finetune.FineTuneConfig`),
...
def _load_eval_records(path: Path, limit: int | None) -> list[dict[str, object]]:
    from linguascrape_ml.finetune import load_jsonl
...
def run(  # pragma: no cover - local-only
    ...
    from linguascrape_ml.finetune import HFCausalLMSystem
