...
* the heavy, device-touching work lives in small functions here that import
  torch/pykeen **lazily** (so ``import linguascrape_ml`` stays cheap and the
...
* the filesystem writes + MLflow logging live in the thin CLI
  :mod:`linguascrape_ml.train_baselines`.
...
def render_baselines_doc(
    ...
    *,
    ...
    lines: list[str] = [
        ...
        " (DVC-tracked / git-ignored respectively).",
        "",
        "## Corpus & split version",
        "",
        "| Artifact | Hash |",
        "| --- | --- |",
        f"| Canonical export (`export/culturescrape`, DVC md5) | `{corpus_md5}` |",
    ...
    lines += [
        ...
        "```",
        "",
        "Metrics are logged to the `linguascrape-baselines` MLflow experiment"
