...
:mod:`linguascrape_ml.export_verbalizations`.

Design mirrors :mod:`linguascrape_ml.triples`:

* **Derived temporal relations are excluded** (via
  :data:`~linguascrape_ml.triples.EXCLUDED_RELATIONS`) — they are Datalog rules
...
from linguascrape_ml.triples import EXCLUDED_RELATIONS
