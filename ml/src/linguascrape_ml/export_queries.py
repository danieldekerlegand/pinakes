...
Reproducer for US-002 of the Scallop pilot. Run from anywhere::
    ...
    uv run python -m linguascrape_ml.export_queries

or via the console script::

    uv run linguascrape-export-queries

Reads the DVC-tracked triples splits (``ml/data/triples/{train,valid,test}.tsv`` +
``triples.tsv`` — themselves built by :mod:`linguascrape_ml.export_triples`), draws
...
from linguascrape_ml.consistency import load_edge_constraints, parse_predictions
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_file.exists():
        parser.error(
            ...
            "`uv run linguascrape-export-triples` first."
        )
