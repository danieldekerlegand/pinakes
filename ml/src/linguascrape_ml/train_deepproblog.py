...
Reproducer for Phase-5 US-004 (the neurosymbolic pilot's go/no-go). Run from ``ml/``::
    ...
    uv run python -m linguascrape_ml.train_deepproblog
    # or the console script:
    uv run linguascrape-deepproblog
...
* sweeps corpus subset sizes and records where exact knowledge compilation ceils out
  (:func:`~linguascrape_ml.deepproblog_pilot.scale_probe`) — the scale ceiling,
  measured;
...
and gated (:func:`~linguascrape_ml.deepproblog_pilot.require_deepproblog_deps`), the
...
from linguascrape_ml.consistency import parse_predictions
