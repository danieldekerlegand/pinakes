...
* :func:`load_relations` reads the canonical **edge** export
  ...
  lists — the same header-driven parse as :mod:`linguascrape_ml.triples`, keyed by
...
* :func:`transitive_closure` is the engine-free reference derivation the scallopy
  smoke run is spot-checked against (:mod:`linguascrape_ml.export_scallop`).
...
:mod:`linguascrape_ml.export_scallop`.
...
from linguascrape_ml.triples import load_triples
...
def load_relations(edges_dir: Path | str) -> dict[str, list[tuple[str, str]]]:
    ...
    Reuses :func:`linguascrape_ml.triples.load_triples` (the same header-driven,
...
def build_scl(translated: list[TranslatedRule]) -> str:
    ...
    lines: list[str] = [
        ...
        "// linguascrape_ml.export_scallop — do not edit by hand.",
        "",
    ]
