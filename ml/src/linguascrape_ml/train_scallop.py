...
run is local-only — it needs the DVC-tracked embeddings/splits/queries)::
    ...
    uv run python -m linguascrape_ml.train_scallop
    # or the console script:
    uv run linguascrape-train-scallop
...
(:mod:`linguascrape_ml.scallop_train`); evaluates filtered MRR/Hits@k on the
...
:func:`linguascrape_ml.scallop_train.build_scl_program`; running it in real
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not embeddings_path.exists():
        parser.error(
            ...
            "baselines with `uv run linguascrape-train-baselines`) first."
        )
