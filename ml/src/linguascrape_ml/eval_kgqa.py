...
the DVC-tracked corpus + eval split — ``uv run --project ml dvc pull`` first)::
    ...
    uv run python -m linguascrape_ml.eval_kgqa
    # or the console script:
    uv run linguascrape-eval-kgqa

It loads the registered held-out split (:data:`~linguascrape_ml.export_kgqa.
...
from linguascrape_ml.consistency import load_edge_constraints
from linguascrape_ml.export_kgqa import KGQA_EVAL_PATH
from linguascrape_ml.kgqa import build_graph
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.eval_split.exists():
        parser.error(
            ...
            "with `uv run linguascrape-export-kgqa`) first."
        )
