...
committed TSV export exactly like :mod:`linguascrape_ml.triples` and
:mod:`linguascrape_ml.verbalize`), so tests drive it with tiny temp-dir fixtures
and the build is byte-reproducible. The thin CLI + MLflow logging live in
:mod:`linguascrape_ml.export_kgqa`; the OPTIONAL Gemini-polish pass (a clearly
separated variant, never required for CI) lives in
:mod:`linguascrape_ml.polish_kgqa` and reuses :func:`apply_polish` here.

Design mirrors :mod:`linguascrape_ml.verbalize`:

* **Derived temporal relations are excluded** (via
  :data:`~linguascrape_ml.triples.EXCLUDED_RELATIONS`) — reused through
  :func:`~linguascrape_ml.verbalize.load_edge_rows`, which drops them.
...
from linguascrape_ml.verbalize import NodeInfo, load_edge_rows, load_nodes
