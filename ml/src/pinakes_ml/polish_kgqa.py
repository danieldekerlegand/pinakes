...
unchanged answer). The deterministic core dataset (``pinakes-export-kgqa``)
...
Run against an existing split::
    ...
    GEMINI_API_KEY=… uv run pinakes-polish-kgqa \\
        --input  ml/data/kgqa/eval.jsonl \\
        --output ml/data/kgqa/eval.polished.jsonl

The pure transform (:func:`~pinakes_ml.kgqa.apply_polish`) is unit-tested
...
from pinakes_ml.kgqa import apply_polish
...
def _gemini_polisher(model_name: str):  # pragma: no cover - networked
    ...
    if not api_key:
        raise SystemExit(
            ...
            "dataset (pinakes-export-kgqa) needs no key."
        )
    try:
