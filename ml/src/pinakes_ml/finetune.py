...
through the existing tier-3 scorer (:mod:`pinakes_ml.kgqa_eval`) so the run
...
* **A pure, dependency-light core** — dataset assembly (JSONL -> instruction
  ...
  *scoring* wiring are all pure functions of their inputs with **no heavy imports
  at module load**, so ``import pinakes_ml.finetune`` works in the slim CI
...
The thin CLI + MLflow logging live in :mod:`pinakes_ml.train_qlora`; the
...
def evaluate_systems(
    ...
    Reuses the US-004 :func:`~pinakes_ml.kgqa_eval.score_system` so a
