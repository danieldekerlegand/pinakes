...
Reproducer for Phase-5 US-001. Run from anywhere::
    ...
    uv run python -m pinakes_ml.export_scallop            # build artifacts
    uv run pinakes-export-scallop                         # console script
    uv run python -m pinakes_ml.export_scallop --smoke    # + real scallopy run
...
(:func:`pinakes_ml.scallop.reference_derivations`). ``scallopy`` is a heavy,
...
DEFAULT_REGISTRY = (
    _REPO_ROOT / "engine" / "src" / "pinakes_engine" / "datalog" / "rules_registry.tsv"
)
