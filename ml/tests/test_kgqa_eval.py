...
from linguascrape_ml.eval_kgqa import build as build_report_cli
from linguascrape_ml.kgqa import build_examples, build_graph
...
def _node_row(csid: str, name: str) -> str:
    return "\t".join([csid, name, "", "", "", "", "linguascrape", "", "", "CC-BY-4.0"])


def _edge_row(head: str, tail: str, rel: str) -> str:
    return "\t".join([head, tail, rel, "linguascrape", "", "", "CC-BY-4.0"])
...
def test_committed_baseline_matches_live_corpus() -> None:
    ...
    from linguascrape_ml.kgqa_eval import DEFAULT_RETRIEVAL_DEPTH
