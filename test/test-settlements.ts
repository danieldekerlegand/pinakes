...
async function testSettlementsLoader() {
  ...
  assert(capitals.every((s) => s.type === "capital"), "all filtered results have type=capital");
  ...
  assert(
    mesopotamia.every((s) => s.region.toLowerCase().includes("mesopotamia")),
  ...
  assert(
    europeBox.every((s) => s.latitude >= 35 && s.latitude <= 60 && s.longitude >= -10 && s.longitude <= 40),
  ...
  assert(
    romanCiv.every((s) => s.civilizationId.toLowerCase() === "roman-empire"),
  ...
  assert(
    nearRome.some((s) => s.id === "rome-settlement"),
